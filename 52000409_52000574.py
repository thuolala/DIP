...
# Function for filling missing color 
def fill(img):
    # Detect contours
    contours, hie = cv.findContours(image=img, mode=cv.RETR_EXTERNAL, method=cv.CHAIN_APPROX_SIMPLE) 
    for cnt in contours:
        img = cv.drawContours(img, contours, contourIdx=-1, color=(255,255,255),thickness=-1)
    return img
...
res = cv.bitwise_and(input1, input1, mask = fill(mask))
...
# # Re-attacht that area with original 
